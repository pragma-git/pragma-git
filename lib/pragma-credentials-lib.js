//
// Functions related to git credentials on LOCAL system
//
...
//
// NOTE: 
// The git credentials are stored on local system.  
// The reason for this is that many git credential helpers must run from GUI, and thus do not work for git over ssh
// (which thus cannot query the credential helper on the ssh server side).  Instead I handle credentials locally.

const pragmaCredentialsRunInFolder = '/';  // I am not sure why this works and CwD_INIT fails.
...
async function storeUsernameInLocalGitConfig( remoteUrl){ // Stores the git credentials username in file=".git/config" key=credential.username
    // This works both for normal and for ssh-folder
...
async function getCredential( remoteUrl){  // Git credentials for single repo as struct
    ...
    try{
        ...
        console.log(cmd);
        credentialFieldsString = await multiPlatformExecSync( pragmaCredentialsRunInFolder, cmd, forcelocal = false, mode = 'timeout', 1000); 
...
async function setCredential( remoteUrl, user, password){  // Set git credentials (NOTE : Do not overwrite.  Use clearCredential first.
    ...
    try{
        ...
        console.log(cmd);
        credentialFieldsString = await multiPlatformExecSync(pragmaCredentialsRunInFolder, cmd); 
    }catch(err){
