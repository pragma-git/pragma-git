...
async function getCredential( remoteUrl){  // Git credentials for single repo as struct
    ...
    try{
        ...
        if (repoInfo.localFolder.startsWith('ssh:') ){
            ...
            cmd=`export  GIT_ASKPASS=''; echo "url=${url}" | ${SSH_GIT_CLIENT} -PIPED  -c "SSHURL=${repoInfo.localFolder}" -c "include.path=${configFileNormalized}" credential fill`;
        }
        
        console.log(cmd);
        credentialFieldsString = await multiPlatformExecSync( repoInfo.localFolder , cmd, mode = 'timeout', 1000); 
...
async function clearCredential( remoteUrl){  // Clear git credential
    ...
    try{
        configFile = configFilePath();   // Assumes that this file is imported into app.js
        let configFileNormalized = configFile.replaceAll('\\','/');
        let cmd=`echo "url=${remoteUrl}" | git -c "include.path=${configFileNormalized}" credential reject`;
        
        if (repoInfo.localFolder.startsWith('ssh:') ){
            SSH_GIT_CLIENT= STARTDIR + pathsep + 'ssh_folder' + pathsep + 'ssh-git-client'; 
            cmd=`export  GIT_ASKPASS=''; echo "url=${url}" | ${SSH_GIT_CLIENT} -PIPED  -c "SSHURL=${repoInfo.localFolder}" -c "include.path=${configFileNormalized}" credential reject`;
        }
...
async function setCredential( remoteUrl, user, password){  // Set git credentials
    ...
    try{
        ...
        let cmd=`echo "url=${remoteUrl}\nusername=${user}\npassword=${password}\n\n "| git -c "include.path=${configFileNormalized}" credential approve`;
        
        if (repoInfo.localFolder.startsWith('ssh:') ){
            SSH_GIT_CLIENT= STARTDIR + pathsep + 'ssh_folder' + pathsep + 'ssh-git-client'; 
            cmd=`export  GIT_ASKPASS=''; echo "url=${url}" | ${SSH_GIT_CLIENT} -PIPED  -c "SSHURL=${repoInfo.localFolder}" -c "include.path=${configFileNormalized}" credential approve`;
        }
