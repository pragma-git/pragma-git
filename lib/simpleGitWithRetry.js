 ...
 *     const git = simpleGit('./my-repo', { maxRetries: 3 });
 *     await git.add('.'); // Will retry automatically on index.lock errors
 */
    function simpleGitWithRetry(baseDir, retryOptions = {}) {
            ...
            // Create the original simple-git instance
            const gitInstance = simpleGitOriginal(baseDir);
                    ...
                    for (let attempt = 1; attempt <= config.maxRetries; attempt++) {
                        try {
                            ...
                            if (shouldRetry && attempt < config.maxRetries) {
                                await wait(config.delay * attempt);
                                continue;
                            }
